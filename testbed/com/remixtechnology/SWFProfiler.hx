...
class SWFProfiler {
	...
	public static function init(?main = null) : Void {
		if(inited) return;
	...
	private static function select_ci(_) : Void {
		if(!displayed) {
			show();
		} else {
			hide();
	...
	private static function select_gc_ci(_) : Void {
		System.gc();
	...
	private static function frameLoop(_) : Void {
...
class ProfilerContent extends Sprite {
	...
	public function update(runningTime : Float, minFps : Int, maxFps : Int, minMem : Float, maxMem : Float, currentFps : Int, currentMem : Float, averageFps : Float, fpsList : Array<Int>, memList : Array<Float>, history : Int) : Void {
