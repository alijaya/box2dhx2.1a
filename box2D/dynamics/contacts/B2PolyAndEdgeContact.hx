...
class B2PolyAndEdgeContact extends B2Contact {
	...
	function B2CollidePolyAndEdge(manifold: B2Manifold,
	                                                ...
	                                                xf2: B2Transform): Void
	{
		...
		var separationMax1: Number = -box2D.common.math.B2Math.MAX_VALUE; // the shallowest depth of edge in poly
		var separation2: Number;
		var separationIndex2: int = -1; // which normal on the poly found the shallowest depth?
		var separationMax2: Number = -box2D.common.math.B2Math.MAX_VALUE; // the shallowest depth of edge in poly
		var separationMax: Number = -box2D.common.math.B2Math.MAX_VALUE; // the shallowest depth of edge in poly
		...
		var deepestSepN: Number = box2D.common.math.B2Math.MAX_VALUE; // the depth of the deepest poly vertex under the end (stored as a separation, so it's negative)
		...
		if (dirProj1 > edge.m_length) {
			...
			if (ratio > 100.0 * box2D.common.math.B2Math.MIN_VALUE && ratio < 1.0) {
