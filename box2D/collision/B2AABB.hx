...
class B2AABB
{
	...
	*/
	...
	{
		var result:Bool = (lowerBound.x <= aabb.lowerBound.x)
	...
	{
		var tmin:Float = -box2D.common.math.B2Math.MAX_VALUE;
		var tmax:Float = box2D.common.math.B2Math.MAX_VALUE;
		...
		//x
		{
			if (absDX < box2D.common.math.B2Math.MIN_VALUE)
			{
				// Parallel.
		...
		//y
		{
			if (absDY < box2D.common.math.B2Math.MIN_VALUE)
			{
				// Parallel.
	...
	public static function Combine(aabb1:B2AABB, aabB2:B2AABB):B2AABB
	{
		...
		aabb.SetCombine(aabb1, aabB2);
		return aabb;
	}
	
	/** Combine two AABBs into one. */
	public function SetCombine(aabb1:B2AABB, aabB2:B2AABB):Void
	{
